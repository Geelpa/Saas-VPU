...
function gerarTabelaVencimentos() {
    ...
    vencimentos.forEach(dia => {
        const dataVenc = new Date(ano, mes, dia);
        const dataBloq = new Date(dataVenc);
        dataBloq.setDate(dataBloq.getDate() + 15);

        const dataMax = new Date(dataBloq);
        ...
        <tr>
          ...
          <td style="padding: 10px; border: 1px solid #ccc;">${formatar(dataBloq)}</td>
